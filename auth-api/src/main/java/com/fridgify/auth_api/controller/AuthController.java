...
public class AuthController {
    ...
    public AuthController(
        JwtUtil jwtUtil,
        UserServiceClient userServiceClient1
    ) {
    ...
    public ResponseEntity<Optional<ResponseUserDTO>> register(
    ...
    public ResponseEntity<TokenDTO> login(
        ...
        try {
            ResponseEntity<Optional<ResponseUserDTO>> response =
                userServiceClient.loginUser(loginUserDTO);
            assert Objects.requireNonNull(response.getBody()).isPresent();
            // Generate JWT token
            String token = jwtUtil.generateToken(loginUserDTO.getUsername());
            TokenDTO tokenResponse = TokenDTO
                .builder()
                .token(token)
